...
import { getAvailableDestinations, Destination } from '../services/destinationService';
...
const HomePage: React.FC = () => {
  ...
  const handleBookNowClick = (destination: Destination) => {
    if (!isAuthenticated) {
      ...
      navigate('/login', { state: { from: location, action: 'book', destinationId: destination._id } });
    } else {
  ...
  useEffect(() => {
    if (isAuthenticated && location.state?.action === 'book' && location.state?.destinationId) {
  ...
  }, [isAuthenticated, location.state, destinations, navigate, location.pathname]);


  if (isLoading) {
    return <div className="loading-message">Loading available destinations...</div>;
  }
  ...
  if (destinations.length === 0) {
    return <div className="no-destinations-message">No available destinations at the moment. Please check back later!</div>;
  }
        ...
        {destinations.map((destination) => (
          <div key={destination._id} className="destination-card">
            ...
            <div className="destination-info">
              ...
              <p className="destination-price">${destination.price.toFixed(2)}</p>
              <button 
