...
const API_BASE_URL = "http://localhost:3001/api/bookings"; // Update to use full URL with backend port
...
export const getAllBookingsAdmin = async (): Promise<Booking[]> => {
  try {
    ...
    if (!token)
      throw new Error("Admin token not found. Cannot fetch all bookings.");
...
export const cancelBookingAdmin = async (
  ...
  try {
    const token = getAuthToken();
    if (!token)
      throw new Error("Admin token not found. Cannot cancel booking.");
    ...
    console.error(
      `Error cancelling booking with ID ${bookingId} for admin:`,
      error
    );
    if (axios.isAxiosError(error) && error.response) {
      throw new Error(
...
export const getMyBookings = async (): Promise<Booking[]> => {
  try {
    const token = getAuthToken();
    if (!token)
      throw new Error("User not authenticated. Cannot fetch bookings.");
    ...
    throw new Error(
      "Failed to fetch your bookings due to an unexpected error."
    );
  }
};
