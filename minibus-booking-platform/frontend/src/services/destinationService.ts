...
const API_URL = '/api/destinations'; // API endpoint for destinations
...
export type DestinationData = Omit<Destination, '_id' | 'createdAt' | 'updatedAt'>;
...
export const getDestinations = async (): Promise<Destination[]> => {
  try {
    ...
    if (!token) console.warn('Admin token not found for getDestinations. API call may fail.');
...
export const createDestination = async (destinationData: DestinationData): Promise<Destination> => {
  try {
    const token = getAuthToken();
    if (!token) throw new Error('Admin token not found. Cannot create destination.');
    ...
    const response = await axios.post<Destination>(API_URL, destinationData, config);
    return response.data;
  } catch (error) {
...
export const updateDestination = async (id: string, destinationData: DestinationUpdateData): Promise<Destination> => {
  try {
    const token = getAuthToken();
    if (!token) throw new Error('Admin token not found. Cannot update destination.');
    ...
    const response = await axios.put<Destination>(`${API_URL}/${id}`, destinationData, config);
    return response.data;
  } catch (error) {
...
export const deleteDestination = async (id: string): Promise<{ message: string }> => {
  try {
    const token = getAuthToken();
    if (!token) throw new Error('Admin token not found. Cannot delete destination.');
...
export const getAvailableDestinations = async (): Promise<Destination[]> => {
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      throw new Error(error.response.data.message || 'Failed to fetch available destinations');
    }
    throw new Error('Failed to fetch available destinations due to an unexpected error.');
  }
};
